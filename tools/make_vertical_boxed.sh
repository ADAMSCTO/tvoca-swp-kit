...
# Notes: - 9:16 vertical (1080x1920) PlayRes
...
# 3) Ensure 9:16 PlayRes and inject/replace CenterBox style
#    (BackColour format AABBGGRR; black with alpha BOX_OPA)
if ! grep -q '^PlayResX:' "$ASS"; then
  sed -i '1{/^\[Script Info\]$/!q}; t; a PlayResX: 1080\nPlayResY: 1920' "$ASS"
fi
sed -i -E 's/^PlayResX:.*/PlayResX: 1080/; s/^PlayResY:.*/PlayResY: 1920/' "$ASS"
...
# 5) Render vertical MP4 with ASS
